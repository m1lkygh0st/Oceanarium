...
void Aquarium::addWorker(const Worker &worker) { workers.push_back(worker); }
int Aquarium::getWorkerCount() const { return static_cast<int>(workers.size()); }

Worker &Aquarium::getWorker(const int index) { return workers.at(index); }

const std::vector<Worker> &Aquarium::getWorkers() const { return workers; }


void Aquarium::feedFishByWorker(const Worker &worker) const {
  std::cout << "Worker " << worker.getName() << " is feeding fish in Aquarium " << aquariumNumber << "\n";
